...
import { WebGLMath, MathTypes } from './math';
...
export class WebGLOrbitCamera {
    static DEFAULT_DISTANCE = 5.0;
    static DEFAULT_MIN_DISTANCE = 1.0;
    static DEFAULT_MAX_DISTANCE = 10.0;
    static DEFAULT_MOVE_SCALE = 2.0;
    ...
    maxDistance: number;
    moveScale: number;
    position: MathTypes['vec3'];
    center: MathTypes['vec3'];
    upDirection: MathTypes['vec3'];
    defaultPosition: MathTypes['vec3'];
    defaultCenter: MathTypes['vec3'];
    defaultUpDirection: MathTypes['vec3'];
    movePosition: MathTypes['vec3'];
    rotateX: number;
    rotateY: number;
    scale: number;
    isDown: boolean;
    prevPosition: MathTypes['vec2'];
    offsetPosition: MathTypes['vec2'];
    qt: MathTypes['qtn'];
    qtx: MathTypes['qtn'];
    qty: MathTypes['qtn'];
    ...
    constructor(
        target: HTMLElement,
        option: { distance?: number; min?: number; max?: number; move?: number } = {}
    ) {
        this.target = target;
    ...
    mouseInteractionStart(event: MouseEvent): void {
        ...
        this.prevPosition = Vec2.create(event.clientX - bound.left, event.clientY - bound.top);
    }
    ...
    mouseInteractionMove(event: MouseEvent): void {
        ...
        switch (event.buttons) {
            case 1: // 左ボタン
                this.rotateX = (this.rotateX + this.offsetPosition[0] * s) % 1.0;
                this.rotateY = Math.min(
                    Math.max((this.rotateY + this.offsetPosition[1] * s) % 1.0, -0.25),
                    0.25
                );
                break;
            case 2: // 右ボタン
     ...
     * マウスボタンが離された際のイベント
     */
    mouseInteractionEnd(_event: MouseEvent): void {
        this.isDown = false;
    }
    ...
    wheelScroll(event: WheelEvent): void {
        this.scale = event.deltaY > 0 ? 0.5 : -0.5;
    }

    /**
     * 現在のパラメータからビュー行列を生成して返す
     * @return {MathTypes['mat4']}
     */
    update(): MathTypes['mat4'] {
